...
EComResponse CCommFB::receiveData(){
  ...
  unsigned int m_unComInterruptQueueCount_init = m_unComInterruptQueueCount;
  for (unsigned int i = 0; i < m_unComInterruptQueueCount_init; ++i){
    eResp = m_apoInterruptQueue[i]->processInterrupt();
    if(eResp > eRetVal){
      eRetVal = eResp;
    }
  }
  m_unComInterruptQueueCount -= m_unComInterruptQueueCount_init;
  for (unsigned int i = 0; i < m_unComInterruptQueueCount; ++i){
	  m_apoInterruptQueue[i] = m_apoInterruptQueue[i + m_unComInterruptQueueCount_init];
  }
  return eRetVal;
}
